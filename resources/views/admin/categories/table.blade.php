...
<div style="padding:24px;">
  <div style="max-width:1100px;margin:0 auto;">
    <div style="display:flex;align-items:baseline;justify-content:space-between;gap:16px;margin-bottom:16px;">
      <div>
        <h3 style="margin:0;font-size:20px;line-height:1.2;font-weight:700;color:#0f172a;">Lista de categorías</h3>
        <p style="margin:4px 0 0 0;font-size:13px;color:#475569;">Administra y organiza las categorías de productos.</p>
      </div>
      <div>
    ...
    <div style="border-radius:14px;border:1px solid #e5e7eb;background:#ffffff;box-shadow:0 1px 3px rgba(0,0,0,.06),0 1px 2px rgba(0,0,0,.03);overflow:hidden;">
      <div style="padding:14px 16px;border-bottom:1px solid #f1f5f9;background:#f8fafc;">
        <div style="display:flex;align-items:center;justify-content:space-between;">
          <span style="font-size:14px;color:#334155;font-weight:600;">Categorías</span>
          <span style="font-size:12px;color:#64748b;">Total: {{ $categories->total() }}</span>
        </div>
      </div>
          ...
          <tbody>
            @foreach($categories as $idx => $category)
              <tr style="background:{{ $idx % 2 === 0 ? '#ffffff' : '#fcfcfd' }};transition:background .2s ease;"
                ...
                <td style="padding:12px 10px;color:#0f172a;font-size:14px;">{{ $category->id }}</td>
                <td style="padding:12px 10px;color:#0f172a;font-size:14px;font-weight:600;">{{ $category->name }}</td>
                <td style="padding:12px 10px;color:#334155;font-size:13px;">{{ $category->created_at }}</td>
                <td style="padding:12px 10px;color:#334155;font-size:13px;">{{ $category->updated_at }}</td>
            ...
            <tr>
              <td colspan="5" style="padding:10px;border-top:1px solid #e2e8f0;background:#fafafa;">
                <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
                  <span style="font-size:12px;color:#64748b;">Mostrando {{ $categories->firstItem() }}–{{ $categories->lastItem() }} de {{ $categories->total() }}</span>
                  <div style="display:flex;align-items:center;gap:6px;">{{ $categories->links() }}</div>
                </div>
              </td>
            </tr>
          </tfoot>
